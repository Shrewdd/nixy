{
  ...
  programs.ghostty = {
    ...
    settings = {
      ...
      # Neovide-like smooth cursor trail animation
      custom-shader = lib.mkDefault "${pkgs.writeText "cursor-warp.glsl" ''
        // --- CONFIGURATION ---
        vec4 TRAIL_COLOR = iCurrentCursorColor;
        const float DURATION = 0.15;
        const float TRAIL_SIZE = 0.9;
        ...
        float antialising(float distance, float blurAmount) {
            return 1.0 - smoothstep(0.0, normalize(vec2(blurAmount, blurAmount), 0.0).x, distance);
        }
        ...
        void mainImage(out vec4 fragColor, in vec2 fragCoord) {
            ...
            if (lineLength > minDist && baseProgress < DURATION - 0.001) {
                ...
                vec2 fragVec = vu - normalize(previousCursor.xy, 1.0);
                vec2 moveVecNorm = normalize(moveVec, 0.0);
                float fadeProgress = clamp(dot(fragVec, moveVecNorm) / (dot(moveVecNorm, moveVecNorm) + 1e-6), 0.0, 1.0);
      ...
      # Keep cursor blinking during shader animation
