...
ShellRoot {
  ...
  FileView {
    ...
    onLoadFailed: {
      // Fallback: try nvidia-smi path or other hwmon paths
      root.gpuTemp = "-- °C"
      root.gpuTempRaw = "--"
    }
  }
  ...
  PanelWindow {
    ...
    // Let PanelWindow auto-detect screen
    exclusionMode: ExclusionMode.Ignore
    
    implicitWidth: mainRow.implicitWidth
    implicitHeight: mainRow.implicitHeight
    
    // Center positioning when component loads
    ...
    // Show function for external access
    function show() {
      visible = true
      // requestActivate not available on PanelWindow
    }
    
    // Hide function for external access  
    function hide() {
      visible = false
    }
    
    // Note: Keys can't be attached to PanelWindow, handle via other means
    ...
    RowLayout {
      ...
      Rectangle {
        ...
        // Alternative shadow effect using border
        border.width: 1
        ...
        ColumnLayout {
          ...
          Rectangle {
            ...
            Rectangle {
              ...
              radius: parent.radius
              gradient: Gradient {
                GradientStop { position: 0.0; color: root.yellowColor }
          ...
          Label {
            ...
            font.family: "SF Pro Display, Inter, system-ui"
            color: {
              if (root.cpuTempRaw === "--") return root.subTextColor
              const temp = parseInt(root.cpuTempRaw)
              if (temp > 85) return "#e67e80"
              if (temp > 75) return root.yellowColor 
      ...
      Rectangle {
        ...
        ColumnLayout {
          ...
          Label {
            ...
            font.family: "SF Pro Display, Inter, system-ui"
            color: {
              if (root.gpuTempRaw === "--") return root.subTextColor
              const temp = parseInt(root.gpuTempRaw)
              if (temp > 80) return "#e67e80"
              if (temp > 70) return root.yellowColor 
